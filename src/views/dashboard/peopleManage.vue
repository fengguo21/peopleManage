  ...
  <div class="container">
    <div class="selectedPersons" @click="toggle">
      <div class="personsIncharge">
        <span v-if="personsIncharge.length>0" class="title">负责人:</span>
        <div v-for="item in personsIncharge" class="peopleInfo">
          <img
            class="avator"
            :src="item.avator"
          ><span
      ...
      <span v-if="collaborators.length>0" class="title">协作者:</span>

      <div class="Collaborator">
        <div v-for="(item,index) in collaborators" v-if="index<3" :key="item.name" class="peopleInfo">
          <img
      ...
      <span v-if="collaborators.length>3" class="extra">+{{ collaborators.length -3 }}</span>
    </div>
    <div v-show="showPannel" class="allPeoples">
      <div class="searchBox">
        <i class="el-icon-search"/>
        <input v-model="keyword" class="search" placeholder="搜索用户..." @input="search($event)">
      ...
      <div
        ...
        class="people"
        :class="{ isPersonIncharge: item.isPersonIncharge, 'isCollaborator': item.isCollaborator }"
      >
        ...
        <div class="right">
          ...
          <div v-if="item.isPersonIncharge || item.isCollaborator" class="del">
            ...
            <span class="tag" @click="unSet(index)">X</span>
          </div>
...
export default {
  ...
  methods: {
    ...
    setPersonIncharge(index) {
      ...
      this.filterPersons[index].isPersonIncharge = true
    },
    setCollaborator(index) {
      this.filterPersons[index].isCollaborator = true
    },
    unSet(index) {
...
<style lang="less" scoped>
  ...
  .allPeoples {
    ...
    position: absolute;
    top: 40px;
    box-shadow: 0px -2px 2px 0px #eee, /*上边阴影  红色*/ -2px 0px 2px 0px #eee, /*左边阴影  绿色*/ 2px 0px 2px 0px #eee, /*右边阴影  蓝色*/ 0px 2px 0px 0px #eee;
